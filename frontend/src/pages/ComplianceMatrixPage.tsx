...
export const ComplianceMatrixPage: React.FC = () => {
  ...
  const filterTabs = [
    { id: 'ALL', label: 'All Requirements', count: results.length },
    { id: 'COMPLIANT', label: 'Compliant', count: results.filter(r => r.status === 'COMPLIANT').length },
    { id: 'NON_COMPLIANT', label: 'Non-Compliant', count: results.filter(r => r.status === 'NON_COMPLIANT').length },
    { id: 'UNVERIFIED', label: 'Unverified', count: results.filter(r => r.status === 'UNVERIFIED').length },
    { id: 'CONTRADICTIONS', label: 'Contradictions & Variances', count: contradictionItems.length },
  ];
  ...
  const getStatusBadge = (status: ComplianceStatus) => {
    switch (status) {
      ...
      case 'PARTIALLY_COMPLIANT':
        return (
          <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-semibold bg-amber-50 text-amber-700 border border-amber-200">
            <AlertCircle className="w-3.5 h-3.5 text-amber-600" /> Partially Compliant
          </span>
        );
      default:
        return (
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-center gap-4">
          ...
          {isBidder ? (
            <div className="flex items-center gap-2 bg-slate-100 border border-slate-200 px-3 py-1.5 rounded-lg text-xs font-semibold text-slate-900">
              ...
              <span className="text-[10px] bg-slate-200 text-slate-800 px-1.5 py-0.5 rounded font-mono font-bold">OWN BID SCOPED</span>
            </div>
          ) : (
            <div className="flex items-center gap-2">
              <Building2 className="w-4 h-4 text-slate-600 shrink-0" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-xs">
              ...
              <div className="flex items-end gap-3">
                ...
                <span className={`px-2.5 py-1 rounded-full text-xs font-bold mb-1 border ${
                  complianceScore.riskLevel === 'LOW' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' :
              ...
              <div className="grid grid-cols-4 gap-2 mt-4">
                {[
                  { label: 'Compliant', value: complianceScore.compliantCount, color: 'text-emerald-700 bg-emerald-50 border border-emerald-100' },
                  { label: 'Non-Compliant', value: complianceScore.nonCompliantCount, color: 'text-rose-700 bg-rose-50 border border-rose-100' },
                  { label: 'Unverified', value: complianceScore.unverifiedCount, color: 'text-amber-700 bg-amber-50 border border-amber-100' },
                  { label: 'Pending Review', value: complianceScore.pendingHumanReviewCount, color: 'text-slate-700 bg-slate-100 border border-slate-200' },
                ].map(item => (
          ...
          {aiRecommendation && (
            <div className={`border rounded-xl p-5 shadow-xs ${
              aiRecommendation.recommendationType === 'RECOMMEND_QUALIFY' ? 'bg-emerald-50/60 border-emerald-200' :
              aiRecommendation.recommendationType === 'RECOMMEND_REJECT' ? 'bg-rose-50/60 border-rose-200' :
              ...
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className={`ml-auto px-2 py-0.5 rounded-full text-[10px] font-bold border ${
                  aiRecommendation.recommendationType === 'RECOMMEND_QUALIFY' ? 'bg-emerald-100 text-emerald-800 border-emerald-300' :
                  aiRecommendation.recommendationType === 'RECOMMEND_REJECT' ? 'bg-rose-100 text-rose-800 border-rose-300' :
              ...
              <p className="text-[10px] text-slate-500 mt-3 italic border-t border-slate-200/80 pt-2">
        ...
        <div className="bg-amber-50/80 border border-amber-200 rounded-xl p-4 flex items-start gap-3 text-amber-900 shadow-2xs">
          ...
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              ...
              <span className="px-2 py-0.2 rounded text-[10px] font-bold bg-amber-100 text-amber-800 border border-amber-300">FLAGGED FOR OFFICER REVIEW</span>
            </div>
      ...
      {loading ? (
        ...
        /* Main Matrix Layout: Self-start left panel sizes dynamically to content */
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
          {/* Compliance Table (Left Panel - Self Start prevents artificial height stretching) */}
          <div className="lg:col-span-1 border border-slate-200 rounded-xl bg-white shadow-xs overflow-hidden self-start">
            <div className="p-4 bg-slate-900 text-white flex items-center justify-between">
              <h2 className="text-xs font-bold flex items-center gap-2 tracking-wide uppercase">
                <FileText className="w-4 h-4 text-slate-300" /> Requirements vs Verification Status
              </h2>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between text-[10px] text-slate-500 font-mono">
                      <span>Engine: {r.verificationMethod || 'DETERMINISTIC'}</span>
          ...
          {/* Reclaimed Space Details Drawer (Right Panel - 2 cols on lg screens for maximum visual room) */}
          {selectedResult && (
            <div data-tour="reasoning-chain" className="lg:col-span-2 bg-white rounded-xl border border-slate-200 shadow-xs p-6 space-y-6 flex flex-col">
              ...
              <div className="border-b border-slate-100 pb-4">
                <div className="flex items-center justify-between gap-2 mb-1">
                  ...
                  <span className="text-xs text-slate-500 font-medium">
                    Category: <strong>{selectedResult.category || 'Technical'}</strong>
                  </span>
                </div>
                ...
                <div className="mt-3 flex items-center justify-between bg-slate-50 p-3 rounded-xl border border-slate-200">
                  <div>
                    <span className="text-[10px] text-slate-400 block uppercase font-bold tracking-wider">Current Verification Status</span>
                    <div className="mt-1">{getStatusBadge(selectedResult.status)}</div>
                  </div>
                  <div className="text-right">
                    <span className="text-[10px] text-slate-400 block uppercase font-bold tracking-wider">Grounding Confidence</span>
              ...
              {/* Full Automated Justification Container */}
              <div className="p-4 bg-slate-50 rounded-xl border border-slate-200 space-y-3">
                <div className="flex items-center gap-2 text-xs font-bold text-slate-800">
                  <Cpu className="w-4 h-4 text-slate-600" />
                  <span>Full Automated Justification</span>
                </div>
                <p className="text-xs text-slate-700 leading-relaxed font-sans">{selectedResult.reasoning}</p>

                <div className="pt-3 border-t border-slate-200 space-y-2">
                  <span className="text-[11px] font-bold text-slate-600 block">Auditable Evidence Citations</span>
                    ...
                    {selectedResult.evidenceCitations && selectedResult.evidenceCitations.length > 0 ? (
                      ...
                      <div className="space-y-1">
                        <div className="flex items-center justify-between text-slate-700">
                          ...
                          <span className="bg-slate-100 text-slate-700 border border-slate-200 text-[10px] px-2 py-0.5 rounded font-mono font-bold">
                            {selectedResult.verificationMethod || 'DETERMINISTIC'}
                          </span>
                        </div>
                        <p className="text-[11px] text-slate-600 font-sans italic bg-slate-50 p-2 rounded border border-slate-200/60">
              ...
              {/* Primary Action Area: Procurement Officer Human Review Card */}
              {isBidder ? (
                <div className="p-4 bg-slate-100 rounded-xl border border-slate-200 text-xs text-slate-800 space-y-2">
                  <div className="flex items-center gap-2 font-bold text-slate-900 text-xs">
                    <ShieldCheck className="w-4 h-4 text-slate-700" /> Official Bidder Verification Record
                  </div>
                ...
                <div className="p-4 bg-slate-100 rounded-xl border border-slate-200 text-xs text-slate-800 space-y-2">
                  <div className="flex items-center gap-2 font-bold text-slate-900 text-xs">
                    <Lock className="w-4 h-4 text-slate-700" /> Vigilance Audit Oversight (Read-Only)
                  </div>
                  <p className="text-[11px] text-slate-600 leading-relaxed">
                ...
                <div data-tour="officer-override-card" className="bg-slate-50/90 border-2 border-indigo-200/80 ring-1 ring-indigo-500/10 shadow-md rounded-xl p-5 space-y-4">
                  <div className="flex items-center justify-between border-b border-indigo-100 pb-3">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <h3 className="text-sm font-bold text-slate-900">Procurement Officer Human Review</h3>
                        <p className="text-[11px] text-slate-500">Statutory binding decision under GFR 2017 Clause 144</p>
                      </div>
                    </div>
                  ...
                  <form onSubmit={handleOverrideSubmit} className="space-y-3">
                    <div>
                      <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Set Decision Status *</label>
                      <select
                      ...
                      >
                        <option value="COMPLIANT">Valid Compliant — Satisfies Requirement Criteria</option>
                        <option value="PARTIALLY_COMPLIANT">Partially Compliant — Minor Deficiency Noted</option>
                        <option value="NON_COMPLIANT">Non-Compliant — Disqualified / Unmet</option>
                        <option value="UNVERIFIED">Unverified — Pending Secondary Audit</option>
                        <option value="NOT_APPLICABLE">Not Applicable — Clause Exemption Granted</option>
                      </select>
                    </div>

                    <div>
                      <div className="flex items-center justify-between mb-1">
                        <label className="text-xs font-bold text-slate-700 uppercase tracking-wider">Reviewer Note (Auditable Justification) *</label>
                        <span className={`text-[10px] font-mono ${overrideNote.trim().length >= 15 ? 'text-emerald-600 font-bold' : 'text-slate-400'}`}>
                          {overrideNote.trim().length}/15 min chars
                        </span>
                      </div>
                      <textarea
                        rows={3}
                        ...
                        placeholder="Enter statutory justification for approving or overriding AI recommendation..."
                      ...
                      <p className="text-[10px] text-slate-500 mt-1">
                        This written justification will be permanently anchored to the Ethereum audit ledger.
                      </p>
                    </div>
                    ...
                    >
                      {submitting ? 'Recording & Anchoring Review...' : 'Submit Auditable Override'}
