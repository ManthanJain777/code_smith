-- V5: Synthetic Demo Dataset — All 10 Test Cases (A–J) from Master Briefing
-- 3 Tenders × 2 Bidders, covering every evaluation scenario

-- ═══════════════════════════════════════════════════════
-- TENDER 1: Industrial Pump Procurement (Primary Demo)
-- ═══════════════════════════════════════════════════════
INSERT INTO tenders (id, organization_id, tender_number, title, description, issuing_authority, category, estimated_value, status, created_by)
VALUES (
...
-- ═══════════════════════════════════════════════════════
-- BIDDER A: Apex Pumps & Motors Pvt Ltd — Mixed Results
-- ═══════════════════════════════════════════════════════
INSERT INTO bids (id, tender_id, bidder_name, bidder_gstin, bidder_pan, bidder_email, status, risk_score, debarment_status)
VALUES ('BID-APEX-001', 'TND-PUMP-001', 'Apex Pumps & Motors Pvt Ltd', '07AAAAA0000A1Z5', 'AAACA1234F', 'apex@apexpumps.com', 'UNDER_EVALUATION', 65.0, 'CLEAR')
;

-- Compliance results for Apex (Case B=NON_COMPLIANT, A=COMPLIANT, C=UNVERIFIED, D=PARTIALLY)
...
VALUES
-- Case D: PARTIALLY_COMPLIANT — 2 of 3 years meet turnover
...
-- Case E: Contradiction flagged (800 vs 500 units/day)
...
-- Case C: UNVERIFIED — no government order found
...
-- Case J: NON_COMPLIANT — ISO cert expired before submission date
...
-- Case I: COMPLIANT after unit normalization (145 PSI = 9.99 Bar >= 10 Bar check)
...
-- ═══════════════════════════════════════════════════════
-- BIDDER B: GlobalFlow Engineers Ltd — Clearly Compliant
-- ═══════════════════════════════════════════════════════
INSERT INTO bids (id, tender_id, bidder_name, bidder_gstin, bidder_pan, bidder_email, status, risk_score, debarment_status)
VALUES ('BID-GFL-001', 'TND-PUMP-001', 'GlobalFlow Engineers Ltd', '29BBBBB1111B2Z6', 'BBACA5678G', 'bid@globalflow.in', 'UNDER_EVALUATION', 15.0, 'CLEAR')
;
...
-- ═══════════════════════════════════════════════════════
-- TENDER 2: IT Hardware Procurement
-- ═══════════════════════════════════════════════════════
INSERT INTO tenders (id, organization_id, tender_number, title, description, issuing_authority, category, estimated_value, status, created_by)
VALUES ('TND-IT-002', 'ORG-001', 'GEM/2026/IT/30210', 'Supply of Servers and Network Equipment', 'Procurement of rack-mounted servers (min 64GB RAM) and enterprise network switches.', 'National Informatics Centre', 'IT Equipment', 80000000.00, 'OPEN', 'USR-PROC-01')
;
...
-- ═══════════════════════════════════════════════════════
-- TENDER 3: Office Furniture
-- ═══════════════════════════════════════════════════════
INSERT INTO tenders (id, organization_id, tender_number, title, description, issuing_authority, category, estimated_value, status, created_by)
VALUES ('TND-FURN-003', 'ORG-001', 'GEM/2026/F/10050', 'Supply of Ergonomic Office Furniture', 'Office chairs and desks meeting BIS standards for 500 workstations.', 'Department of Science & Engineering', 'Office Furniture', 15000000.00, 'OPEN', 'USR-PROC-01')
;
